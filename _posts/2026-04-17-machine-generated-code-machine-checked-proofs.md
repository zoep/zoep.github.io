...
- **Automath** (Nicolaas Govert de Bruijn, 1967). One of the first proof assistants with a formal language and a proof checker. Also, the first proof assistant based on dependent type theory and the Curry-Howard correspondence, which is the basis for many modern proof assistants.
- **Logic for Computable Functions (LCF)** (Robin Milner, 1970s). LCF was an automated and interactive theorem prover. It gave rise to the ML programming language, which was introduced as a meta-language for writing proof tactics.
...
We are at a point that most of us working in the field could not have anticipated. Large language models, systems trained on vast corpora of text and code, have developed the ability to reason mathematically and generate proofs in formal systems that can be mechanically checked for correctness by the same proof assistants that researchers have been building for decades.
...
My role was to direct the process: I would point it at the next proof obligation to tackle. In a few situations where the proof got stuck, I had to figure out the underlying reasoning and provide guidance on how to tackle the proof, in natural language. But I did not write any proof code myself. The LLM handled all the plumbing: setting up inductions, performing case analyses, applying lemmas from the codebase and the Rocq standard library.
...
- I have used LLMs to extend the aforementioned ANF proof to a) prove a stronger property (i.e., that a transformation preserves non-terminating behavior) and b) handle a more complex source language. In both cases, the LLM was able to adapt the existing proof to the new setting, but not without frustration and limitations. During this experiment, I also experimented with other models, namely GPT-5.4 with Codex, which I found to be way more effective for the particular task.
...
As the preliminary results suggest, this vision is not far from reality. However, there are still challenges to overcome.
